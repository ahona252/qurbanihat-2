## DIGITAL QURBANIHAT

# 🚀 purposes :
- Hassle-Free Buying is one of the biggest goal of this platform.

- Weight-Based Pricing is one of the speciality.

- Fraud Prevention, by using this platform we can get rid of frauds.

🌻 Essentially, the purpose of a Digital QurbaniHat is to transform a deeply rooted religious tradition into a modern, safe, transparent, and hassle-free e-commerce experience that benefits both the consumer and the agricultural community.

# LIVE URL: 

# KEY FEATURES :

🚚 Doorstep Delivery
💰 Cashless Payment
📸 Details proper photos
🌟 Every Details of the animal (Breed,weight,age etc)

# Technologies & npm packages :

- Next.js
- Mongodb Atlas
- Tailwind 
- HeroUI
- React-Toastify